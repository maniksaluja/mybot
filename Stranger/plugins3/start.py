...
from pyrogram.errors import FloodWait, MessageIdInvalid, InputUserDeactivated, UserIsBlocked, MediaEmpty, MessageNotModified

from Stranger import userbot, shotner
from Stranger.plugins.tools.reaction_post import trigger_reaction_post_update
from Stranger.utils.database.mongodatabase import add_post_reaction, update_order, get_post_by_content_id
from Stranger.utils.helper import generate_unique_token, get_readable_time, retry_with_flood_wait
            ...
            if try_backup_1:
                try:
                    ...
                    if try_backup_1 and is_that_userbot_one:
                        await temp_msg.edit("Trying for backup files ....")
                    ...
                    if try_backup_2 and is_that_userbot_two:
                        ...
                        if backup_result == "processing":
                            return await temp_msg.edit("⏳ Backup files are currently being processed. Please try again in a few minutes.")
                        elif backup_result == "started":
                            return await temp_msg.edit("🔄 Backup processing started. Please try again in 2-5 minutes while we prepare your files.")
...
async def refresh_payment(client:Client, callback_query:CallbackQuery):
    ...
    if result['status'] == ['paid']:
        ...
        return callback_query.answer(text="Something went wrong!!!")
